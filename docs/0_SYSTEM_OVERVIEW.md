...
The host project itself is **Django Pro Template**: a production-ready Django 5.2 backend boilerplate. Its stated design goal (per the root `README.md`) is a secure, scalable starting point — dynamic path/config loading via `config.toml` + `.env`, hardened security defaults, structured logging, and a single-app skeleton (`apps.core`) ready to be extended.
...
backend/            Django project (WSGI app via gunicorn in production).
  ...
  apps/core/         The only installed local app — currently an empty skeleton
                     (models.py, views.py, admin.py are stubs) plus a real
                     PasswordComplexityValidator used by AUTH_PASSWORD_VALIDATORS.
...
## 6. Full inventory
For the detailed component-by-component map (what lives where inside `.agents/`, current status of each piece), read `.agents/docs/architecture/topology_map.md`. For this project's own inventory, see `docs/architecture/CORE_BLUEPRINT.md` and `docs/active_state.json`'s `topology_map` key.

## 7. Onboarding note
This project predates the `.agents` framework (Onboarding Scenario C: mature project, no prior agentic traces). `docs/architecture/CORE_BLUEPRINT.md` and `docs/walkthroughs/CORE_WALKTHROUGH.md` were generated by Full Reverse Engineering of the existing codebase, not by executing a Sprint. No Sprint folder exists yet — the first one opens at the next Planning phase.
